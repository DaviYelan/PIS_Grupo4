...
socket.on("total_mensaje_sin_leer", function (data_total_mensaje_sin_leer) {
  ...
  ulElement.querySelectorAll("li").forEach((liElement) => {
    ...
    if (memberId === parseInt(data_total_mensaje_sin_leer.desde_id_user)) {
      //Mover mensaje a recientes
      const desde = data_total_mensaje_sin_leer.desde_id_user;
      let idQuienEnviaMsj = `userId_${desde}`;
      posiciona_nuevo_msj_recibido(idQuienEnviaMsj);
...
/**
 * Posicicionar el nuevo mensaje recibido
 */

const posiciona_nuevo_msj_recibido = (idQuienEnviaMsj) => {
  var elementoDeseado = document.querySelector(
    `li[data-user="${idQuienEnviaMsj}"]`
  );

  if (elementoDeseado) {
    const lista = document.querySelector("ul.messages-page__list"); // Selecciona la lista <ul>
    if (lista) {
      lista.insertBefore(elementoDeseado, lista.firstElementChild);
    }
  }
};
/**
